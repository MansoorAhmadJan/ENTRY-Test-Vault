  ...
  function render(container) {
    ...
    container.innerHTML = `
      ...
      <div class="section csp-5d37f2">
        <div class="card">
          <div class="section-title csp-7f5713"><strong>Resources by Subject</strong></div>
          ${barChart(Array.from(bySubject, ([label, value]) => ({ label, value })).sort((a, b) => b.value - a.value))}
        </div>
        <div class="card">
          <div class="section-title csp-7f5713"><strong>Resources by University</strong></div>
      ...
      <div class="section csp-5d37f2">
        <div class="card">
          <div class="section-title csp-7f5713"><strong>Most Used Resources</strong></div>
          <div id="stats-most-used"></div>
        </div>
        <div class="card">
          <div class="section-title csp-7f5713"><strong>Your Favorites</strong></div>
    ...
    if (!mostUsed.length) {
      mostUsedHost.innerHTML = `<p class="csp-923776">Open a few resources to see your most-used list build up here.</p>`;
    } else {
      mostUsedHost.innerHTML = mostUsed
        .map(
          (x) => `
        <div class="filter-bar csp-663510">
          <span class="csp-bfdc35" data-open="${x.resource.id}">${App.Utils.escapeHtml(x.resource.title)}</span>
    ...
    if (!favoriteResources.length) {
      favHost.innerHTML = `<p class="csp-923776">No favorites yet.</p>`;
    } else {
      favHost.innerHTML = favoriteResources
        .map(
          (r) => `
        <div class="filter-bar csp-663510">
          <span class="csp-bfdc35" data-open="${r.id}">${App.Utils.escapeHtml(r.title)}</span>
