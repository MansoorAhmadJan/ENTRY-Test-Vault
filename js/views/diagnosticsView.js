...
(function (App) {
  ...
  function renderDataIntegrity(container, report) {
    container.innerHTML = `
      <div class="stat-grid csp-04c235">
        <div class="stat-card"><div class="stat-value">${report.totalResources}</div><div class="stat-label">Resources Scanned</div></div>
        <div class="stat-card"><div class="stat-value csp-200ea6">${report.bySeverity.error || 0}</div><div class="stat-label">Errors</div></div>
        <div class="stat-card"><div class="stat-value csp-e705c6">${report.bySeverity.warning || 0}</div><div class="stat-label">Warnings</div></div>
        <div class="stat-card"><div class="stat-value csp-03f9c8">${report.bySeverity.info || 0}</div><div class="stat-label">Info</div></div>
      </div>
      ${
      ...
      `
          : `
        <div class="card csp-a73fd8">
          <table class="csp-5dd2f7">
            <thead>
              <tr class="csp-2e77f2">
                <th class="csp-9a1ee1">Severity</th>
                <th class="csp-9a1ee1">Category</th>
                <th class="csp-9a1ee1">Resource</th>
                <th class="csp-9a1ee1">Message</th>
              </tr>
            </thead>
            <tbody>
              ${report.issues
                .map(
                  (i) => `
                <tr class="csp-a56f95">
                  <td class="csp-9a1ee1">${severityBadge(i.severity)}</td>
                  <td class="csp-9a1ee1">${App.Utils.escapeHtml(i.category)}</td>
                  <td class="csp-3f382f">${App.Utils.escapeHtml(i.resourceId)}</td>
                  <td class="csp-9a1ee1">${App.Utils.escapeHtml(i.message)}</td>
                </tr>`
                )
                .join("")}
            </tbody>
  ...
  function renderSystemHealth(container) {
    ...
    container.innerHTML = `
      <div class="self-test-summary ${passCount === results.length ? "pass" : "fail"}" class="section--mb4">
  ...
  function renderPerformance(container) {
    ...
    container.innerHTML = `
      <div class="stat-grid csp-04c235">
        <div class="stat-card"><div class="stat-value">${startupDurationMs ?? "—"}<span class="csp-9e6595">ms</span></div><div class="stat-label">Total Startup Time</div></div>
        <div class="stat-card"><div class="stat-value">${health.lastBuildDurationMs}<span class="csp-9e6595">ms</span></div><div class="stat-label">Search Index Build Time</div></div>
        <div class="stat-card"><div class="stat-value">${dataKb ?? "—"}<span class="csp-9e6595">KB</span></div><div class="stat-label">Vault Data Size (JSON)</div></div>
        <div class="stat-card"><div class="stat-value">${storageKb ?? "—"}<span class="csp-9e6595">KB</span></div><div class="stat-label">Local Storage Used</div></div>
      </div>

      <div class="card csp-04c235">
        <div class="section-title csp-7f5713"><strong>Startup Timeline</strong></div>
        ${marks
          .map(
            (m, i) => `
          <div class="filter-bar csp-663510">
            <span class="csp-d7674c">${App.Utils.escapeHtml(m.name)}</span>
      ...
      <div class="card">
        <div class="section-title csp-7f5713"><strong>Search Index Health</strong></div>
  ...
  function renderAiAndStorage(container) {
    ...
    container.innerHTML = `
      <div class="card csp-04c235">
        <div class="section-title csp-7f5713"><strong>AI Provider Status</strong></div>
        <div class="meta-table">
      ...
      <div class="card csp-04c235">
        <div class="section-title csp-7f5713"><strong>Storage Usage</strong></div>
        <div class="progress-bar csp-663510"><div class="progress-bar-fill" data-fill="${quota.pct}"></div></div>
        <p class="csp-af5552">${quota.usedKb} KB used of an estimated ~${(quota.limitKb / 1024).toFixed(0)} MB typical browser limit (${quota.pct}%) — actual limits vary by browser and aren't directly queryable, so this is a conservative estimate, not a guarantee.</p>
        ${breakdown
          .slice(0, 12)
          .map(
            (row) => `
          <div class="filter-bar csp-663510">
            <span class="csp-f18566">${App.Utils.escapeHtml(row.key)}</span>
            <span class="badge badge-outline">${row.kb} KB</span>
          </div>`
          )
          .join("")}
        ${breakdown.length === 0 ? `<p class="rm-muted-hint">No data stored yet.</p>` : ""}
      </div>

      <div class="card">
        <div class="section-title csp-7f5713"><strong>Build Information</strong></div>
        <div class="meta-table">
