...
(function (App) {
  ...
  function suggestionItemHtml(s) {
    return `<div class="sd-history-item" data-suggestion="${App.Utils.escapeHtml(s.term)}">${App.Icons.get("search")}<span>${App.Utils.escapeHtml(s.term)}</span><span class="csp-675591">${s.count}</span></div>`;
  }
    ...
    root.innerHTML = `
      ...
      <div class="header-search">
        <div class="search-box">
          ...
          <button class="clear-btn" id="global-search-clear" aria-label="Clear search">${App.Icons.get("x")}</button>
        </div>
        ...
        const gotoRoute =
          topic.type === "university"
            ...
            : topic.type === "subject"
              ...
              : null;
        html += `<div class="sd-topic-hint topic-hint-bar ${gotoRoute ? "is-clickable" : ""}" ${gotoRoute ? `data-goto-topic="${gotoRoute}" role="button" tabindex="0"` : ""}>Related topic: <strong class="topic-hint-label">${App.Utils.escapeHtml(topic.label)}</strong>${gotoRoute ? " — click to browse" : ""}</div>`;
      }
